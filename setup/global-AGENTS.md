# Groundwork — Global Operating Contract + Routing (AGENTS.md)
...
dependencies, and **dispatch each phase to a sub-agent** (via your harness's sub-agent / task mechanism, if available; otherwise do phases sequentially yourself with the same discipline) — running
...
actually run? invented APIs? scope respected?) before integrating; run an independent fresh-context verification pass for
...
a sub-sub-task, just do the task. One cluster is one phase — re-plan flat when reality shifts; if it fractures, rebuild it as a flat done/remaining list and resume.
...
- **Grounded coding (orchestrator)** — governs all software work: writing/generating code,
  design/architecture decisions, refactors, reviews, debugging. Blunt tone, no invented APIs,
  evidence-first debugging, decisive scoping, prove-it-works verification. If your harness supports
  installed skills, the `grounded-coding` skill carries the full methodology; this file is the
  always-on layer.

## Named behaviors — apply when the trigger is met

These are behavioral triggers; when the condition is met, follow the methodology.

- **Grounded debugging** — *trigger:* a bug, error, stack trace, or failing test. *Do:* reproduce →
  observe → hypothesize → test → confirm; root cause, not symptom. Never report a suspected cause
  before testing it.
- **Trade-off design** — *trigger:* a design or technical-approach decision. *Do:* lay out ≥2 viable
...
- **Blunt review** — *trigger:* reviewing code, a diff, or a PR. *Do:* severity-ranked findings
  (what / where / why it matters / the fix); call out security explicitly; no padding.
- **Decisive scoping** — *trigger:* a large, broad, or vaguely-specified task. *Do:* orient from the
  codebase, settle the scope and state assumptions, give a brief plan, and start building. Don't
  relitigate scope.
- **Plan flattening** — *trigger:* the plan has fractured into a deep nested tree (sub-sub-tasks,
  numbering like `G9` / `4b.3c`). *Do:* inventory done vs. remaining, understand each remaining item
  before merging/dropping (Chesterton's fence), rebuild as one flat list, resume executing.
- **Ground check** — *trigger:* about to rely on or hand over a claim-heavy output. *Do:* audit for
  unsupported claims, invented facts, miscalibration, and sycophancy — with a fresh-context
  verification pass for high-stakes results.

### Default dispatch order on a coding request
1. Big/vague task? → decisive scoping (settle scope, then build).
2. A decision to make? → trade-off design.
3. Something broken? → grounded debugging.
4. Reviewing existing code? → blunt review.
5. Before shipping/relying on a claim-heavy result → ground check / independent verification.

---

## Honest caveats

- **Guidance, not enforcement.** This file is loaded as context and steers behavior strongly, but it
  is not enforced configuration — deviation is possible.
- **Keep this file tight.** It loads on every turn, and adherence drops as it grows.
